...
async function auditOne(url: string, opts: any) {
  let slug: string;
  let cleanUrl: string;
  try {
  ...
  const last = (state as any)[slug]?.lastScanned;
...
program
  ...
  .option('--all', 'Re-audit all tracked plugins to latest master (from data/state.json)')
  .action(async (url: string | undefined, opts) => {
    if (opts.all) {
      const allState = await readState();
      const entries = Object.entries(allState);
      if (entries.length === 0) {
        console.log('No plugins tracked yet. Add one with: omarchy-audit <github-url>');
        return;
      }
      console.log(pc.cyan(`Re-auditing ${entries.length} tracked plugins to latest master${opts.withLlm ? ' with AI' : ''}...`));
      let ok = 0, failed = 0;
      for (const [slug, entry] of entries) {
        const u = (entry as any).url as string;
        console.log(pc.cyan(`\n[${ok+failed+1}/${entries.length}] ${u} (${slug})`));
        try {
          await auditOne(u, { ...opts, force: true });
          ok++;
        } catch (e: any) {
          failed++;
          console.error(pc.red(`Failed ${u}: ${e.message}`));
        }
      }
      console.log(pc.green(`\nDone: ${ok} succeeded, ${failed} failed`));
      if (failed === 0) {
        console.log(pc.cyan('Rebuilding site...'));
        try {
          const { spawn } = await import('node:child_process');
          await new Promise<void>((res, rej) => {
            const p = spawn('pnpm', ['--filter', '@omarchy-audit/site', 'build'], { stdio: 'inherit' });
            p.on('close', (c) => (c === 0 ? res() : rej(new Error(`site build failed ${c}`))));
            p.on('error', rej);
          });
          console.log(pc.green('Site rebuilt to dist/'));
        } catch {}
      }
      return;
    }
